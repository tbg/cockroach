...
This is  is optimized for low overhead, and powers fine-grained statistics and
alerts. Note that this mode is not compatible with CockroachDB v20.2 (trace
information will be lost), so it should only be used once the cluster version
is at least v21.1. Note also that configuring an auxiliary trace sinks will
continue to receive verbose traces, which may lead to high memory consumption.
It is not currently possible to send non-verbose traces to auxiliary sinks.
