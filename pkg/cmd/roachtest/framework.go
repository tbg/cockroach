...
type Generator struct {
	m map[string]Stepper
	w map[string]float64
}

func (r *Generator) Register(s Stepper, w float64) {
	name := s.Name()
	r.m[name] = s
	r.w[name] = w
}
...
type RandStepIter interface {
	Next(r *rand.Rand) (Stepper, Instance, bool)
}

func Run(t Fataler, c Cluster, it RandStepIter) {
	...
	for {
		...
		var m *monitor // TODO should be suitably baked into Cluster
		if !opts.Chaos() {
