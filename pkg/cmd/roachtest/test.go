...
func (r *registry) prepareSpec(spec *testSpec) error {
	...
	if err := r.verifyTestClusterName(spec.Name); err != nil {
		return err
	}
...
//
// Args:
// clusterLifetimeOverride: If not 0, this value will be used to override
//   testSpec.Nodes[0].Lifetime for all tests. Useful, for example, when running
//   a large number of tests that can reuse clusters and the default 12h
//   destruction time for those clusters is not desirable.
func (r *registry) Run(
	ctx context.Context,
	filter *testFilter,
	count int,
	...
	if clusterLifetimeOverride != 0 {
		for _, t := range tests {
			t.Cluster.Lifetime = clusterLifetimeOverride
		}
	}
	...
	shout(ctx, l, stdout, "test runner logs in: %s", runnerLogPath)

	// Shut down test clusters when interrupted (for example CTRL-C).
	sig := make(chan os.Signal, 1)
	signal.Notify(sig, os.Interrupt)
	// We're going to run all the tests in a context gets canceled the the
	// Interrupt signaled is received.
	...
	for i := 0; i < parallelism; i++ {
		...
		stopper.RunWorker(ctx, func(ctx context.Context) {
			if err := r.runWorker(
				...
				shout(ctx, l, stdout, "Worker %d returned with error. Quiescing. Error: %s", i, err)
				errs.AddErr(err)
	...
	case <-doneCh:
		...
		// over for that should be explicitly marked in the registry.
		// !!! stdout and the web page display nothing for this step. Maybe they
		// should.
		destroyAllClusters(ctx, l)
	case <-sig:
		// Notify all the workers to stop by canceling their contexts and also by
		// closing the resourceGovernor (for the workers that are blocked on
		// acquiring resources).
		// We'll then wait for 5s for all the workers to finish and, if they don't,
		// we destroy the clusters from under them.
		shout(ctx, l, stderr, "Signaled received. Canceling workers and waiting up to 5s for them.")
		cancel()
		select {
		case <-doneCh:
		...
		go func() {
			destroyAllClusters(context.Background() /* non-canceled ctx */, l)
			close(destroyCh)
		}()
	...
	if errs.Err() != nil {
		shout(ctx, l, stdout, "FAIL (err: %s)", errs.Err())
		return 1
	}
	// !!! this can produce an empty line
...
func (r *registry) runWorker(
	...
	allocateCluster := func(
		...
		if existingClusterName != "" {
			...
			return attachToExistingCluster(ctx, existingClusterName, clusterL, t.Cluster, opt)
		}
	...
	defer func() {
		...
		if t.Failed() {
			...
			if postIssues && issues.CanPost() && t.spec.Run != nil {
				...
				if err := issues.Post(
					context.Background(),
					// !!! see if I should pass anything for extraLabels
					fmt.Sprintf("roachtest: %s failed", t.Name()),
					"roachtest", t.Name(), "The test failed on "+branch+":\n"+string(output), authorEmail, nil, /* extraLabels */
				); err != nil {
