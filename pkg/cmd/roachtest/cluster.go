...
func (r *clusterRegistry) tagCluster(tag string) {
	r.mu.Lock()
	r.mu.tagCount[tag]++
	r.mu.Unlock()
}
		...
		for _, c := range clusters {
			go func(c *cluster) {
				...
				if _, ok := savedClusters[c]; !ok {
					...
					c.Destroy(context.Background(), dontCloseLogger, l)
				}
			}(c)
		}
...
// closed upon return.
//
// Cluster destruction errors are swallowed. The cluster's quota is always
// released, even on errors.
func (c *cluster) doDestroy(ctx context.Context, l *logger) <-chan struct{} {
	...
	// !!! there's a bug here if the ctx is canceled. The guy with the canceled
	// ctx sets destroyed, and then everybody else will just join in this request
	// - which request will fail after a few secs because the ctx is canceled.
...
// Tag applies a tag to the cluster.
func (c *cluster) Tag(tag string) {
	c.tag = tag
	c.r.tagCluster(tag)
}
