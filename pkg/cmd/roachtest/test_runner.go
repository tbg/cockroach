...
func (r *testRunner) runWorker(
	...
	defer func() {
		...
		// !!! I think I shouldn't attempt to destroy at all if ctx is canceled. I
		// should leave it to the CtrlC.
	...
	for {
		...
		testToRun, c, err = r.getWork(
			...
			func(ctx context.Context, t testSpec, alloc *quotapool.IntAlloc) (*cluster, error) {
...
func (r *testRunner) getWork(
	...
	interrupt <-chan struct{},
	l *logger,
	createCluster func(context.Context, testSpec, *quotapool.IntAlloc) (*cluster, error),
	...
	testToRun, err := work.getTestToRun(ctx, c, qp, r.cr, l)
	if err != nil {
