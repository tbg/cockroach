...
As a database that has to deal with consensus latencies and prides itself on geo-distributed use cases, we have to become optimal in the latency which is incurred. And yet, inserting into virtually any real-world scheme will cost at least one consensus operation more than it really needs to. This is clearly not acceptable and has implications not only in the wide area, but also in clustered deployments.
...
1. the transaction record, when in status STAGED contains the full set of point writes of the txn
...
It motivates a parallel tech note about [changing the format of our transaction IDs](node-txn-ids.md).
